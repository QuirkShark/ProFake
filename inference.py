...
parser.add_argument('--logdir', default='/data/code/deepfake_detection/SLADD-main/logs',
...
# Load your weights
parser.add_argument("--classifier_path",type = str, default='saved/whole-model/ALL/epoch_10_iter_16109_classifier.pth')
parser.add_argument("--synthesizer_path",type = str, default='saved/whole-model/ALL/epoch_10_iter_16109_syn.pth')
parser.add_argument("--backbone_path",type = str, default='saved/whole-model/ALL/epoch_10_iter_16109_backbone.pth')
...
def inference(gpu,args):
    ...
    def get_data_loader(name):
        # -----------------load dataset--------------------------
        test_set = FaceForensicsDataset(dataset=name, mode='test', res=args.resolution, train=False,quality = args.quality)
        test_data_loader = torch.utils.data.DataLoader(test_set, batch_size=args.test_batchSize, shuffle=False, num_workers=int(args.workers))
        return test_data_loader
    test_data_loaders = {}
    for list_key in TESTLIST.keys():
        test_data_loaders[list_key] = get_data_loader(list_key)
